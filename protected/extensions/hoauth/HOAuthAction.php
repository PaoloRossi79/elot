		...
		try{
			...
			else 
			{
				...
				if($accessCode === 1)
				{
					...
					: new $this->userIdentityClass($user->email, $user->password, Yii::app()->params['authExtSource'][$provider]);
	...
	protected function processUser($user, $userProfile)
	{
		...
		if($user->isNewRecord) 
		{
			...
			if(!$user->profile->save())
				throw new Exception("Error, while saving Profile model:\n\n" . var_export($user->profile->errors, true));
	...
	protected function populateModel($user, $profile)
	{
		foreach($this->attributes as $attribute => $pAtt)
		{
			if(in_array($pAtt, $this->_avaibleAtts))
			{
				...
				if(!empty($att)){
                                    $subs=explode("->", $attribute);
                                    if(count($subs) > 1){
                                        $newObj = $user;
                                        for($i=0;$i<count($subs)-1;$i++){
                                            $newObj = $newObj->$subs[$i];
                                        }
                                        $newObj->$subs[count($subs)-1] = $att;
                                    } else {
					$user->$attribute = $att;
                                    }
                                }
